...
if (isset($_POST['submit'])) {
  ...
  if ($id == "") {
    $sql = "INSERT INTO skd (no_skd, nama_skd, tanggal, file_upload)
            VALUES ('$no_skd', '$nama_skd', '$tanggal', '$file')";
  } else {
    $sql = "UPDATE skd SET 
              no_wi='$no_skd',
              nama_wi='$nama_skd',
  ...
  $conn->query($sql);
  header("Location: skd.php");
}

// Hapus
if (isset($_GET['hapus'])) {
  $id = $_GET['hapus'];
  $conn->query("DELETE FROM skd WHERE id=$id");
  header("Location: skd.php");
}

// Edit
$id_edit = "";
$no_skd = $nama_skd = $tanggal = $file_upload = "";
if (isset($_GET['edit'])) {
  $id_edit = $_GET['edit'];
  $row = $conn->query("SELECT * FROM skd WHERE id=$id_edit")->fetch_assoc();
  $no_skd = $row['no_skd'];
  $nama_skd = $row['nama_skd'];
  $tanggal = $row['tanggal'];
...
$allowed_sort = ['no_skd', 'nama_skd', 'tanggal'];
if (!in_array($sort_by, $allowed_sort)) $sort_by = 'id';
$order = strtoupper($order) == 'ASC' ? 'ASC' : 'DESC';

$where = $search ? "WHERE no_skd LIKE '%$search%' OR nama_skd LIKE '%$search%'" : '';
$res = $conn->query("SELECT * FROM skd $where ORDER BY $sort_by $order");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
  ...
  <div class="container">
    <h2><?= $id_edit ? "Edit SKD" : "Input SKD" ?></h2>
    <form method="POST" enctype="multipart/form-data">
      <input type="hidden" name="id" value="<?= $id_edit ?>">
      <input type="text" name="no_skd" placeholder="Nomor SKD" value="<?= $no_skd ?>" required>
      <input type="text" name="nama_skd" placeholder="Nama SKD" value="<?= $nama_skd ?>" required>
    ...
    <form method="GET" class="filter-form">
      <input type="text" name="search" placeholder="Cari SKD..." value="<?= htmlspecialchars($search) ?>">
      <select name="sort_by">
        <option value="tanggal" <?= $sort_by == 'tanggal' ? 'selected' : '' ?>>Sortir Tanggal</option>
        <option value="no_skd" <?= $sort_by == 'no_skd' ? 'selected' : '' ?>>Sortir No SKD</option>
        <option value="nama_skd" <?= $sort_by == 'nama_skd' ? 'selected' : '' ?>>Sortir Nama</option>
      </select>
      <select name="order">
    ...
    <div class="table-wrapper">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td><?= $no++ ?></td>
            <td><?= htmlspecialchars($r['no_skd']) ?></td>
            <td><?= htmlspecialchars($r['nama_skd']) ?></td>
